...
local function getLogPath()
    return pathJoin(getScriptDirectory(), LOG_FILE_NAME)
end
...
local function appendLogRaw(line)
    ...
    local ok = pcall(function()
        local file = assert(io.open(getLogPath(), "ab"))
        file:write(line or "")
        file:flush()
        file:close()
    end)
...
local function logLine(level, message)
    local line = string.format("[%s] [%s] %s\r\n", os.date("%Y-%m-%d %H:%M:%S"), safeToString(level or "INFO"), safeToString(message or ""))
    appendLogRaw(line)
end

local function logError(tag, err)
    local header = safeToString(tag or "error")
    logLine("ERROR", header .. "\n" .. buildTraceback(err, 3))
end

local function logSection(title)
    logLine("INFO", "========== " .. safeToString(title or "") .. " ==========")
end
...
local function getUiStatePath()
    return pathJoin(getScriptDirectory(), STATE_FILE_NAME)
end
...
function main()
    local ok, err = xpcall(function()
        ...
        sampRegisterChatCommand("stats", function()
            logLine("INFO", "chat command /stats")
            if cefVisible then
                closeCefWindow()
                return
            end
