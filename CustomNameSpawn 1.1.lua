...
function main()
    ...
    sampRegisterChatCommand('addcs', function (arg)
        ...
        if not cname or cname == '' then
            sampAddChatMessage(tag..' Впиши нормально второй аргумент!', -1)
            return
        end
            ...
            if idpunkta and namepunkt == 'Дом' and idhouse then
                ...
                for i, hc in ipairs(settings.customName) do
                    ...
                    if idhome == idhouse then
                        nameHouse = nameHome
                        --sampAddChatMessage(string.format('Дом №%s находится в: %s', idhome, nameHome), -1)
                        break
                    end
                end

                if nameHouse == '' then
                    -- Формируем новую строку ТОЛЬКО для формата с "Дом"
