script_name('TestDrive by chapo');
script_author('chapo');
script_version('5.23');
script_url('https://www.blast.hk/threads/109617/');
...
local directIni = 'VisualTestDriveV5.ini';
...
ffi.cdef([[
    ...
    typedef enum MaterialSize MaterialSize;

    typedef struct MaterialText MaterialText;
    #pragma pack(push, 1)
    struct MaterialText {
    ...
    struct CObject{
        ...
        float   field_0;
        struct CVector m_position;
        float   m_fDistanceToCamera;
        bool    m_bDrawLast;
        char    pad_2[64];
        struct CVector m_rotation;
        char    pad_3[5];
        ID      m_nAttachedToVehicle;
        ID      m_nAttachedToObject;
        struct CVector m_attachOffset;
        struct CVector m_attachRotation;
...
local sampVersion = samp == 0 and error('samp.dll not found') or (({ [0x31DF13] = 'R1', [0xCC4D0] = 'R3' })[ffi.cast("unsigned int*", samp + ffi.cast("long*", samp + 60)[0] + 40)[0]] or error('Unknown SA-MP version. Use R1 or R3'));
local CObjectPool = ffi.cast('int(__thiscall*)(void*)', samp + address.objectPool[sampVersion]);
local CObject = ffi.cast('CObject*(__thiscall*)(void*, int)', samp + address.CObject[sampVersion]);
local object = {
    carInfoText = 18663,
    carInfoWall = 19300
};
...
---@return string text
function getObjectMaterialText(objectId)
    local CNetGame = ffi.cast('uintptr_t*', samp + address.CNetGame[sampVersion])
    local objectPool = CObjectPool(ffi.cast('void*', CNetGame[0]))
    local cobject = CObject(ffi.cast('void*', objectPool), objectId)
    return ffi.string(cobject.m_material.m_szText[0])
end
...
local this = {
    ...
    active = false,
    carColors = { ini.main.color1, ini.main.color2 },
    saved = { 
        pos = { 0, 0, 3 },
        heading = 0
    },
...
function this.start(model, x, y, z, heading)
    ...
    this.carHandle = createCar(model, x or this.saved.pos[1], y or this.saved.pos[2], z or this.saved.pos[3]);
...
function this.stop()
    if (this.openMenuAfterExit and not menu.state) then
        menu.switch();
    end
    warpCharFromCarToCoord(PLAYER_PED, table.unpack(this.saved.pos));
...
function this.init()
    addEventHandler('onWindowMessage', function(msgId, param)
    ...
    addEventHandler('onScriptTerminate', function(scr, quit)
        if (scr == thisScript() and this.active) then
            this.stop();
        end
    end);

    --// RakNet Handlers
    for _, event in pairs({ 'onSendRpc', 'onSendPacket' }) do
        addEventHandler(event, function(id, bs)
            if (this.active and (id >= 201 and id <= 212)) then
                return false;
            end
            if (this.active and event == 'onSendRpc' and id == 119) then
                setCarCoordinates(this.carHandle, raknetBitStreamReadFloat(bs), raknetBitStreamReadFloat(bs), raknetBitStreamReadFloat(bs))
                return false;
            end
        end);
    end
...
--             if (res) then
--                 local result, handle = sampGetCarHandleBySampVehicleId(data.vehId);
--                 if (result) then
...
local preview = nil;
local vehiclesIde = getGameDirectory() .. '\\data\\vehicles.ide';
local dontLoadModels = imgui.new.bool(false);
local search = imgui.new.char[64]('');
local vehicles = {}
local loadStatus, loadError = 'none', '';
local darkTheme = ini.main.darkTheme;
local renderModelSettings = { background_color = 0x00000000, zoom = 1, rotation = { x = -10, y = 0, z = 45 }, carc_1 = 1, carc_2 = 1 };
...
imgui.OnInitialize(function()
    ...
    colors[imgui.Col.WindowBg] = imgui.ImVec4(0.04, 0.04, 0.04, 1);
    colors[imgui.Col.ChildBg] = imgui.ImVec4(0.04, 0.04, 0.04, 0);
    colors[imgui.Col.FrameBg] = imgui.ImVec4(0.07, 0.07, 0.07, 1);
    ...
    style.ItemSpacing = imgui.ImVec2(15, 15);
    style.FramePadding = imgui.ImVec2(1, 5);

    for name in pairs(Font) do
        for _, size in pairs({ 14, 16, 18, 20, 24 }) do
            ...
            Font[name][size] = imgui.GetIO().Fonts:AddFontFromMemoryCompressedBase85TTF(Font[name].base85, size, nil, range[0].Data);
            ...
            if (faInstalled) then
                ...
                config.GlyphOffset.y = size == 18 and 0 or -2.0;
                ...
                Font[name][size] = imgui.GetIO().Fonts:AddFontFromMemoryCompressedBase85TTF(faicons.get_font_data_base85('solid'), size, config, defaultGlyphRanges[0].Data);
            end
        end
    end
end)
...
---@return table reason
local function loadVehicles(loadModels)
    local status, result = pcall(function()
        local list = {};
        for line in io.lines(vehiclesIde) do
            local thisLine = line:gsub('%s', '');
            -- print(thisLine, thisLine:find('(%d+),(%w+),(%w+),(%w+)'))
            if (thisLine:find('(%d+),(%w+),(%w+),(%w+)')) then
                local modelId, modelName, txdName, class = thisLine:match('(%d+),(%w+),(%w+),(%w+)');
                -- print('MODEL', modelId, modelName, class or 'mnone');
                table.insert(list, {
                    id = tonumber(modelId) or 0,
                    model = modelName,
                    name = modelName,
                    type = class == 'quad' and 'bike' or class,
                    texture = nil,
                    handling = getCarHandling(tonumber(modelId) or 411)
                });
...
local textureLoadDelay, lastTextureLoaded = 0.005, 0;

local function loadVehiclesTextures()
    print('Loading textures...', #vehicles)
    lua_thread.create(function()
        for index, veh in ipairs(vehicles) do
            repeat wait(0) until lastTextureLoaded + textureLoadDelay - os.clock() <= 0;
            lastTextureLoaded = os.clock();
            vehicles[index].texture = render_model(
                tonumber(veh.id) or 411,
                renderModelSettings
            ) or nil;
        end
    end)
...
-- for i = 1, 30 do table.insert(vehicles, { id = i, name = 'placeholder' }) end
imgui.OnFrame(
    function() return isSampAvailable() and menu.alpha > 0.00 end,
    function(thisWindow)
        thisWindow.HideCursor = not menu.state;
        local res = imgui.ImVec2(getScreenResolution());
        local size = imgui.ImVec2(1000, 600);
        imgui.SetNextWindowPos(imgui.ImVec2(res.x / 2, res.y / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5));
        imgui.SetNextWindowSize(size, imgui.Cond.FirstUseEver);
        imgui.PushStyleVarFloat(imgui.StyleVar.Alpha, menu.alpha);
        imgui.PushStyleColor(imgui.Col.WindowBg, darkTheme and imgui.ImVec4(0.04, 0.04, 0.04, menu.alpha) or imgui.ImVec4(0.86, 0.86, 0.86, menu.alpha));
        imgui.PushStyleColor(imgui.Col.ChildBg, darkTheme and imgui.ImVec4(0.07, 0.07, 0.07, menu.alpha) or imgui.ImVec4(0.98, 0.98, 0.98, menu.alpha));
        imgui.PushStyleColor(imgui.Col.FrameBg, darkTheme and imgui.ImVec4(0.07, 0.07, 0.07, menu.alpha) or imgui.ImVec4(0.98, 0.98, 0.98, menu.alpha));
        imgui.PushStyleColor(imgui.Col.Text, darkTheme and imgui.ImVec4(1, 1, 1, menu.alpha) or imgui.ImVec4(0, 0, 0, menu.alpha));
        imgui.PushStyleColor(imgui.Col.Button, darkTheme and imgui.ImVec4(0.04, 0.04, 0.04, menu.alpha) or imgui.ImVec4(0.86, 0.86, 0.86, menu.alpha));
        imgui.PushStyleColor(imgui.Col.ButtonHovered, darkTheme and imgui.ImVec4(0.04 + 0.1, 0.04 + 0.1, 0.04 + 0.1, menu.alpha) or imgui.ImVec4(0.86 - 0.1, 0.86 - 0.1, 0.86 - 0.1, menu.alpha));
        imgui.PushStyleColor(imgui.Col.ButtonActive, darkTheme and imgui.ImVec4(0.04 + 0.15, 0.04 + 0.15, 0.04 + 0.15, menu.alpha) or imgui.ImVec4(0.86 - 0.15, 0.86 - 0.15, 0.86 - 0.15, menu.alpha));
        
        local colors = imgui.GetStyle().Colors;
        if (imgui.Begin('TestDrive', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollWithMouse + imgui.WindowFlags.NoScrollbar)) then
            local pos = imgui.GetWindowPos();
            local DL = imgui.GetWindowDrawList();
            local BGDL = imgui.GetBackgroundDrawList();
            
            DL:AddTextFontPtr(Font.Medium[24], 25, pos + imgui.ImVec2(25, 15), imgui.GetColorU32Vec4(colors[imgui.Col.Text]), faicons('CAR'));
            DL:AddTextFontPtr(Font.Medium[20], 16, pos + imgui.ImVec2(25 + 30, 10), imgui.GetColorU32Vec4(colors[imgui.Col.Text]), 'TestDrive\nby chapo');
            BGDL:AddRectFilled(imgui.ImVec2(0, 0), res, imgui.GetColorU32Vec4(imgui.ImVec4(0, 0, 0, menu.alpha / 2)));
            BGDL:AddTextFontPtr(Font.Medium[20], 20, imgui.ImVec2(10, res.y - 20 - 10), 0xCCffffff, 'TestDrive by chapo | Version: ' .. thisScript().version);
            imgui.SetCursorPos(imgui.ImVec2(25, 50));
            local pos = imgui.GetCursorScreenPos();
            local itemSize = imgui.ImVec2(296, 163);
            imgui.PushFont(Font.Medium[16]);
            DL:AddRectFilled(pos, pos + size - imgui.ImVec2(50, 50), imgui.GetColorU32Vec4(imgui.GetStyle().Colors[imgui.Col.FrameBg]), 15, 1 + 2);
            if (imgui.BeginChild('body', size - imgui.ImVec2(50, 50), false, preview == nil and 0 or (imgui.WindowFlags.NoScrollWithMouse + imgui.WindowFlags.NoScrollbar))) then
                imgui.PushStyleColor(imgui.Col.ChildBg, uiAnimation.themeSwitch and bringVec4To(colors[imgui.Col.WindowBg], imgui.ImVec4(0, 0, 0, 0), uiAnimation.themeSwitch.start, 1) or colors[imgui.Col.WindowBg]);
                imgui.Scroller('vehcilesList', itemSize.y * 1.5, 350, imgui.HoveredFlags.ChildWindows);

                if (#vehicles > 0 and preview == nil) then
                    imgui.SetCursorPos(imgui.ImVec2(15, 15));
                    local displayIndex = 0;
                    for index, veh in ipairs(vehicles) do
                        if ((typeIcon[veh.type] == nil or typeIcon[veh.type].state[0]) and ('%s %s'):format(veh.id, veh.model):lower():find(ffi.string(search):lower(), nil, true)) then
                            displayIndex = displayIndex + 1;
                            if (uiAnimation.selected[index] == nil) then
                                uiAnimation.selected[index] = { hovered = false, start = 0 };
                            end
                            if (imgui.BeginChild('vehicleChild' .. index, itemSize, false, imgui.WindowFlags.NoScrollWithMouse + imgui.WindowFlags.NoScrollbar)) then
                                local thisDL = imgui.GetWindowDrawList();
                                local leftUpCorner = imgui.GetCursorScreenPos();
                                local hovered = imgui.IsMouseHoveringRect(leftUpCorner, leftUpCorner + itemSize);
                                if (hovered ~= uiAnimation.selected[index].hovered) then
                                    uiAnimation.selected[index] = { hovered = hovered, start = os.clock() };
                                end

                                --// Image
                                do
                                    if (veh.texture) then
                                        local imageOffset = bringFloatTo(hovered and 0 or 30, hovered and 30 or 0, uiAnimation.selected[index].start, 0.2);
                                        thisDL:AddImage(veh.texture, leftUpCorner - imgui.ImVec2(imageOffset, 55 + imageOffset), leftUpCorner + itemSize + imgui.ImVec2(imageOffset, 55 + imageOffset), nil, nil, imgui.GetColorU32Vec4(imgui.ImVec4(1, 1, 1, menu.alpha)));
                                    else
                                        local iconSize = imgui.CalcTextSize(faicons('IMAGE'));
                                        thisDL:AddTextFontPtr(Font.Medium[24], 24, leftUpCorner + imgui.ImVec2((itemSize.x - iconSize.y) / 2, (itemSize.y - iconSize.y) / 2), imgui.GetColorU32Vec4(colors[imgui.Col.Text]), faicons('IMAGE'))
                                    end
                                end

                                --// Veh type
                                do
                                    if (typeIcon[veh.type]) then
                                        local icon = (getNameOfVehicleModel(veh.id) == nil and faicons('STAR') .. ' ' or '') .. typeIcon[veh.type].icon;
                                        local iconSize = imgui.CalcTextSize(icon);
                                        local startPos = leftUpCorner + imgui.ImVec2(itemSize.x - iconSize.x - 10, 5);
                                        thisDL:AddRectFilled(startPos, startPos + iconSize + imgui.ImVec2(5, 5), imgui.GetColorU32Vec4(colors[imgui.Col.Text]), 5);
                                        thisDL:AddText(startPos + imgui.ImVec2(1, 5), imgui.GetColorU32Vec4(colors[imgui.Col.Button]), icon);
                                    end
                                end

                                --// Text
                                do
                                    thisDL:AddText(leftUpCorner + imgui.ImVec2(10, 8), imgui.GetColorU32Vec4(colors[imgui.Col.Text]), ('#%s (%s)'):format(veh.id, veh.model));
                                    thisDL:AddText(leftUpCorner + imgui.ImVec2(10, itemSize.y - 14 - 8), imgui.GetColorU32Vec4(colors[imgui.Col.Text]), tostring(veh.name));
                                end

                                --// Hover effect
                                do
                                    local hoverAlpha = bringFloatTo(hovered and 0 or 0.1, hovered and 0.1 or 0, uiAnimation.selected[index].start, 0.3);
                                    thisDL:AddRectFilled(leftUpCorner, leftUpCorner + itemSize, imgui.GetColorU32Vec4(imgui.ImVec4(1, 1, 1, hoverAlpha)), 10);
                                    if (hovered) then
                                        thisDL:AddRect(leftUpCorner, leftUpCorner + itemSize, imgui.GetColorU32Vec4(imgui.ImVec4(1, 1, 1, hoverAlpha)), 10, nil, 3);
                                    end
                                
                                    local selectButtonSize = imgui.ImVec2(128, 28);
                                    local selectButtonYOffset = bringFloatTo(hovered and 0 or 115, hovered and 115 or 0, uiAnimation.selected[index].start, 0.1)
                                    imgui.SetCursorPos(imgui.ImVec2((itemSize.x - selectButtonSize.x) / 2, itemSize.y - selectButtonYOffset));
                                    imgui.BeginGroup();
                                    imgui.PushFont(Font.Medium[18]);
                                    imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.31, 0.76, 0.41, hoverAlpha * 10));
                                    imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.31, 0.76, 0.41, hoverAlpha * 7));
                                    imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.31, 0.76, 0.41, hoverAlpha * 5));
                    ...
                    if (#vehicles == 0) then
                        ...
                        if (imgui.AnimButton(u8'Загрузить', imgui.ImVec2(200, 50))) then
                            ...
                            if (status) then
                                ...
                                if (not dontLoadModels[0]) then
                                    loadVehiclesTextures()
                                end
                            else
                                loadStatus = 'error';
                                loadError = tostring(tempVehicles);
                            end
                            if (true or ffi.string(ffi.C.GetCommandLineA()):find('arizona')) then
                                if (not doesDirectoryExist(getWorkingDirectory() .. '\\resource')) then
                                    createDirectory(getWorkingDirectory() .. '\\resource');
                                end
                                local path = getWorkingDirectory() .. '\\resource\\temp_testDrive_arizonaVehicles.json';
                                downloadUrlToFile(
                                    'https://api.arztools.tech/tools/arizona/vehicles.json',
                                    path,
                                    function (id, status, p1, p2)
                                        if status == require('moonloader').download_status.STATUSEX_ENDDOWNLOAD then
                                            print('arizonaVehicles.json loaded, reading...');
                                            local file = io.open(path, 'r');
                                            if (file) then
                                                local content = file:read('a');
                                                file:close();
                                                local status, result = pcall(decodeJson, content);
                                                print('arizonaVehicles.json - decoded:', result[1].model_id);
                                                if (status and result) then
                                                    for k, v in pairs(result) do
                                                        -- print(v.modelId)
                                                        for index, data in pairs(vehicles) do
                                                            if (v.model_id == data.id) then
                                                                -- sampAddChatMessage('ok' .. v.model_id, -1);
                                                                vehicles[index].name = v.name;
                                                            end
                                                            -- print('compare:', v.model_id, data.id)
                                                        end
                                                    end
                                                else
                                                    print('arizonaVehicles.json - error in decodeJson:', status, result);
                                                end
                                            else
                                                print('arizonaVehicles.json - error in io.open');
                                            end
                                        end
                                    end
                                );
                        ...
                        if (preview and vehicles[preview]) then
                            local veh = vehicles[preview];
                            imgui.NewLine();
                           
                            local imageSize = imgui.ImVec2(500, 500)
                            imgui.SetCursorPos(imgui.ImVec2(0, 0));
                            local p = imgui.GetCursorScreenPos();
                            DL:AddRectFilled(p, p + imgui.ImVec2(imageSize.x, size.y), imgui.GetColorU32Vec4(imgui.GetStyle().Colors[imgui.Col.WindowBg]), 15, 4 + 8)
                            imgui.Image(veh.texture, imageSize)

                            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0, 0, 0, 0));
                            imgui.SetCursorPos(imgui.ImVec2(imageSize.x , 15));
                            if (imgui.BeginChild('handling', imgui.ImVec2(size.x - imageSize.x, size.y - 50), true)) then
                                imgui.PushFont(Font.Medium[20]);
                                imgui.CenterText(('#%d - %s'):format(veh.id, veh.model));
                                imgui.PopFont();
                                imgui.Separator();
                                for k, v in pairs(veh.handling) do
                                    imgui.Columns(2);
                                    imgui.SetColumnWidth(-1, 150);
                                    imgui.Text(u8(tostring(v.name)));
                                    imgui.NextColumn();
                                    imgui.Text(u8(tostring(v.value)));
                                    imgui.Columns(1);
                                end
                                imgui.Separator();
                                imgui.PushFont(Font.Medium[20]);
                                imgui.SetCursorPos(imgui.ImVec2(15, size.y - 100 - 30));
                                imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(1, 1, 1, 1));
                                imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1, 1, 1, 0.7));
            ...
            if (imgui.CircleIconButton(DL, pos + imgui.ImVec2(-15, 9), 8, faicons('CAR'), 'switchTheme', imgui.ImVec4(1, 0.66, 0, menu.alpha), imgui.ImVec4(0.82, 0.59, 0.01, menu.alpha))) then
                uiAnimation.themeSwitch = { dark = darkTheme, start = os.clock() };
            end
            

            --// Search
            imgui.SetCursorPos(imgui.ImVec2(size.x / 2 - size.x / 3 / 2, 12));
            imgui.SetNextItemWidth(size.x / 3)
            ...
            if (imgui.Button(faicons(imgui.IsPopupOpen('filters') and 'CARET_LEFT' or 'CARET_RIGHT'), imgui.ImVec2(28, 28))) then imgui.OpenPopup('filters') end
            imgui.PopStyleColor();
            imgui.PopFont();
            imgui.SameLine();
            ...
            if (imgui.BeginPopup('filters', imgui.WindowFlags.NoMove)) then
                ...
                for k, v in pairs(typeIcon) do
                    if (k ~= 'launcher') then
                        imgui.CustomCheckbox(('%s %s ##filterCheckbox:%s'):format(v.icon, u8(v.label), k), v.state);
                    end
                end
                imgui.NewLine();
            ...
            if (uiAnimation.themeSwitch) then
                local radius = bringFloatTo(0, size.x + 100, uiAnimation.themeSwitch.start, 0.5);
                local alpha = bringFloatTo(0, 1, uiAnimation.themeSwitch.start, 0.5);
                DL:AddCircleFilled(pos + imgui.ImVec2(-15, 9), radius, imgui.GetColorU32Vec4(darkTheme and imgui.ImVec4(1, 1, 1, alpha) or imgui.ImVec4(0.04, 0.04, 0.04, alpha)), 100);
                if (uiAnimation.themeSwitch.start + 0.5 - os.clock() <= 0) then
                    uiAnimation.themeSwitch = nil;
                    darkTheme = not darkTheme;
                    ini.main.darkTheme = darkTheme;
                    inicfg.save(ini, directIni);
                end
            end
            imgui.End()
...
function imgui.TestDriveVehicleColorSelector(colorId)
    imgui.PushStyleColorU32(imgui.Col.Button, carColors[this.carColors[colorId]] or 0xFFffffff)
    if imgui.Button('##changeCarColor'..tostring(colorId), imgui.ImVec2(20, 20)) then
        imgui.OpenPopup('vehicleColorPickerSlot'..tostring(colorId))
    end
    imgui.PopStyleColor()
    if imgui.BeginPopup('vehicleColorPickerSlot'..tostring(colorId)) then
        for id, u32 in ipairs(carColors) do
            imgui.PushStyleColorU32(imgui.Col.Button, u32)
            if imgui.Button(tostring(id)..'##carcoloritem'..colorId, imgui.ImVec2(34, 24)) then
                this.carColors[colorId] = id;
                ini.main['color' .. colorId] = id;
                inicfg.save(ini, directIni);
                -- sampAddChatMessage('color '..colorId..', updated to '..id, -1)
                imgui.CloseCurrentPopup()
            end
            imgui.PopStyleColor()
            if (id + 1) %20 ~= 1 then imgui.SameLine() end
        end
        imgui.EndPopup()
    end
end
...
imgui.OnFrame(
    function() return this.active and isCharInAnyCar(PLAYER_PED) and this.info end,
    function(self)
        self.HideCursor = not isKeyDown(VK_SHIFT);
        if (uiAnimation.expand.state ~= self.HideCursor) then
            uiAnimation.expand = {
                state = self.HideCursor,
                start = os.clock()
            }
        end
        if (isCharInAnyCar(PLAYER_PED)) then
            ...
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.07, 0.07, 0.07, 1));
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.07, 0.07, 0.07, 0.5));
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.07, 0.07, 0.07, 0.3));
            if (imgui.Begin('testdriverprocess', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoScrollbar)) then
                local size = imgui.GetWindowSize();
                local DL = imgui.GetWindowDrawList();
                imgui.PushFont(Font.Medium[24]);
                imgui.CenterText(u8(this.info.modelName or 'undefined'))
                imgui.PopFont();
                
                local speed = ('%03d'):format(math.floor(getCarSpeed(veh) * 3.55));--tostring(math.floor(getCarSpeed(veh) * 3.8));

                if (this.lastSpeedUpdated + 0.1 - os.clock() <= 0) then
                    this.currentSpeed = getCarSpeed(veh) * 3.55;
                    this.lastSpeedUpdated = os.clock();
                end

                imgui.PushFont(Font.Medium[24]);
                -- imgui.CenterText(('%03d'):format(this.currentSpeed or -1));
                local sizeX = 35;
                imgui.SetCursorPosX(size.x / 2 - (sizeX * 3) / 2);
                local p = imgui.GetCursorScreenPos();
                DL:AddRectFilled(p, p + imgui.ImVec2(sizeX * 3, 40), imgui.GetColorU32Vec4(imgui.ImVec4(0.07, 0.07, 0.07, 1)), 10);
                -- DL:AddRect(p, p + imgui.ImVec2(sizeX * 3, 40), 0xFFffffff, 10)
                ...
                for i = 1, 3 do
                    local thisSpeed = speed:sub(i, i);
                    local color = imgui.ImVec4(0.5, 0.5, 0.5, 0.5);
                    if (i == 3 or (i == 2 and tonumber(speed) >= 10) or (i == 1 and tonumber(speed) >= 100)) then
                        color = imgui.ImVec4(1, 1, 1, 1);
                    end
                    
                    imgui.PushStyleColor(imgui.Col.Text, color);
                    imgui.Button(thisSpeed, imgui.ImVec2(sizeX, 40));
                    imgui.PopStyleColor();
                    
                    imgui.SameLine(nil, i < 3 and 0 or 5);
                end
...
function imgui.CircleIconButton(DL, pos, radius, icon, strId, color, colorHover)
    ...
    if (uiAnimation.CircleIconButton[strId] == nil) then uiAnimation.CircleIconButton[strId] = { hovered = hovered, start = os.clock() } end
    if (hovered ~= uiAnimation.CircleIconButton[strId].hovered) then uiAnimation.CircleIconButton[strId] = { hovered = hovered, start = os.clock() } end
    local c = bringVec4To(hovered and color or colorHover, hovered and colorHover or color, uiAnimation.CircleIconButton[strId].start, 0.25)
    DL:AddCircleFilled(pos, radius, imgui.GetColorU32Vec4(c));
...
local function findModelIdByModel(m)
    for _, veh in ipairs(vehicles) do
        local full = ('%s %s %d'):format(veh.name, veh.model, veh.id);
        if (full:lower():find(m:lower())) then
            return veh.id;
        else
            print(full:lower(), m:lower());
        end
    end
    return nil;
end
...
function main()
    ...
    -- wait(5000);
    if (ffi.string(ffi.C.GetCommandLineA()):find('arizona')) then
    ...
    sampRegisterChatCommand('testdrive', function(arg)
        if (#arg == 0) then
            menu.switch();
        else
            if (#vehicles == 0) then
    ...
    lua_thread.create(warpmain)
    while true do
        wait(0);
        if (this.active and doesVehicleExist(this.carHandle)) then
            setCarProofs(this.carHandle, true, true, true, true, true);
            setCarHealth(this.carHandle, 1500);
        end
    end
end
...
function warpmain()
  ...
  while true do
    ...
    if not this.active then cursorEnabled = false end
    if isKeyDown(keyToggle) and this.active then
...
imgui.Scroller = { _ids = {}, _version = 1, _author = "Double Tap Inside" };
...
--// DRAW MODELS IN IMGUI by kin4stat
...
function imgui.CustomCheckbox(str_id, bool, a_speed)
    local p = imgui.GetCursorScreenPos();local DL = imgui.GetWindowDrawList();local label = str_id:gsub('##.+', '') or "";local h = imgui.GetTextLineHeightWithSpacing() + 2;local speed = a_speed or 0.2;local c = {{0.18536826495, 0.42833250947},{0.44109925858, 0.70010380622},{0.38825341901, 0.70010380622},{0.81248970176, 0.28238693976}};if UI_CUSTOM_CHECKBOX == nil then UI_CUSTOM_CHECKBOX = {} end;if UI_CUSTOM_CHECKBOX[str_id] == nil then;UI_CUSTOM_CHECKBOX[str_id] = {lines = {{from = imgui.ImVec2(0, 0),to = imgui.ImVec2(h*c[1][1], h*c[1][2]),start = 0,anim = false,},{from = imgui.ImVec2(0, 0),to = bool[0] and imgui.ImVec2(h*c[2][1], h*c[2][2]) or imgui.ImVec2(h*c[1][1], h*c[1][2]),start = 0,anim = false,},{from = imgui.ImVec2(0, 0),to = imgui.ImVec2(h*c[3][1], h*c[3][2]),start = 0,anim = false,},{from = imgui.ImVec2(0, 0),   to = bool[0] and imgui.ImVec2(h*c[4][1], h*c[4][2]) or imgui.ImVec2(h*c[3][1], h*c[3][2]),start = 0,anim = false,},},hovered = false,h_start = 0,};end;local pool = UI_CUSTOM_CHECKBOX[str_id];imgui.BeginGroup();imgui.InvisibleButton(str_id, imgui.ImVec2(h, h));imgui.SameLine();local pp = imgui.GetCursorPos();imgui.SetCursorPos(imgui.ImVec2(pp.x, pp.y + h/2 - imgui.CalcTextSize(label).y/2));imgui.Text(label);imgui.EndGroup();local clicked = imgui.IsItemClicked();if pool.hovered ~= imgui.IsItemHovered() then;pool.hovered = imgui.IsItemHovered();local timer = os.clock() - pool.h_start;if timer <= speed and timer >= 0 then pool.h_start = os.clock() - (speed - timer) else pool.h_start = os.clock() end;end;if clicked then;local isAnim = false;for i = 1, 4 do if pool.lines[i].anim then isAnim = true end end;if not isAnim then;bool[0] = not bool[0];pool.lines[1].from = imgui.ImVec2(h*c[1][1], h*c[1][2]);pool.lines[1].to = bool[0] and imgui.ImVec2(h*c[1][1], h*c[1][2]) or imgui.ImVec2(h*c[2][1], h*c[2][2]);pool.lines[1].start = bool[0] and 0 or os.clock();pool.lines[2].from = bool[0] and imgui.ImVec2(h*c[1][1], h*c[1][2]) or imgui.ImVec2(h*c[2][1], h*c[2][2]);pool.lines[2].to = bool[0] and imgui.ImVec2(h*c[2][1], h*c[2][2]) or imgui.ImVec2(h*c[2][1], h*c[2][2]);pool.lines[2].start = bool[0] and os.clock() or 0;pool.lines[3].from = imgui.ImVec2(h*c[3][1], h*c[3][2]);pool.lines[3].to = bool[0] and imgui.ImVec2(h*c[3][1], h*c[3][2]) or imgui.ImVec2(h*c[4][1], h*c[4][2]);pool.lines[3].start = bool[0] and 0 or os.clock() + speed;pool.lines[4].from = bool[0] and imgui.ImVec2(h*c[3][1], h*c[3][2]) or imgui.ImVec2(h*c[4][1], h*c[4][2]);pool.lines[4].to = imgui.ImVec2(h*c[4][1], h*c[4][2]) or imgui.ImVec2(h*c[4][1], h*c[4][2]);pool.lines[4].start = bool[0] and os.clock() + speed or 0;        end;end;local pos = {};for i = 1, 4 do pos[i], pool.lines[i].anim = bringVec2To(p + pool.lines[i].from,p + pool.lines[i].to,pool.lines[i].start,speed)end;local color = imgui.GetStyle().Colors[imgui.Col.Text];local c = imgui.GetStyle().Colors[imgui.Col.Text];local colorHovered = bringVec4To(pool.hovered and imgui.ImVec4(c.x, c.y, c.z, 0) or imgui.ImVec4(c.x, c.y, c.z, 0.2),pool.hovered and imgui.ImVec4(c.x, c.y, c.z, 0.2) or imgui.ImVec4(c.x, c.y, c.z, 0),pool.h_start,speed);DL:AddRectFilled(p, imgui.ImVec2(p.x + h, p.y + h), imgui.GetColorU32Vec4(colorHovered), h/15, 15);DL:AddRect(p, imgui.ImVec2(p.x + h, p.y + h), imgui.GetColorU32Vec4(color), h/15, 15, 1.5);DL:AddLine(pos[1], pos[2], imgui.GetColorU32Vec4(color), h/10);DL:AddLine(pos[3], pos[4], imgui.GetColorU32Vec4(color), h/10);;return clicked
end

function imgui.CenterText(text, col)
    imgui.SetCursorPosX(imgui.GetWindowSize().x / 2 - imgui.CalcTextSize(text).x / 2)
    imgui.TextColored(col or imgui.GetStyle().Colors[imgui.Col.Text], text)
end

function bringFloatTo(from, to, start_time, duration)
    local timer = os.clock() - start_time; if timer >= 0.00 and timer <= duration then local count = timer / (duration / 100); return from + (count * (to - from) / 100); end; return (timer > duration) and to or from
end

function bringVec2To(from, to, start_time, duration)
    local timer = os.clock() - start_time;if timer >= 0.00 and timer <= duration then;local count = timer / (duration / 100);return imgui.ImVec2(from.x + (count * (to.x - from.x) / 100),from.y + (count * (to.y - from.y) / 100)), true;end;return (timer > duration) and to or from, false;
end
...
---@param y number
---@param z number
---@return { text: string, dist: number }
function findModelIdPlate(baseObjectId, x, y, z)
    local result = { text = 'undefined', dist = math.huge };
    for _, handle in pairs(getAllObjects()) do
        if (getObjectModel(handle) == object.carInfoText) then
            local posResult, ox, oy, oz = getObjectCoordinates(handle);
            if (posResult) then
                ...
                if (dist < result.dist) then
                    ...
                    if (objectId and objectId ~= baseObjectId) then
                        result.text = getObjectMaterialText(objectId);
                        result.dist = dist;
                    end
                end
            end
...
---@return { dist: number, text: string, pos: number[], vehId: number, modelName: string, fullStr: string }
function getNearestCarInfoTable()
    local result = { dist = math.huge, handle = nil, text = 'undefined', pos = { 0, 0, 0 }, vehId = 1235, modelName = 'undefined' };
    local pedX, pedY, pedZ = getCharCoordinates(PLAYER_PED);
    for _, handle in pairs(getAllObjects()) do
        if (getObjectModel(handle) == object.carInfoText) then
            local posResult, x, y, z = getObjectCoordinates(handle);
            if (posResult) then
                local dist = getDistanceBetweenCoords3d(pedX, pedY, pedZ, x, y, z);
                if (dist < result.dist) then
                    local objectId = sampGetObjectSampIdByHandle(handle);
                    if (objectId) then
                        local objectText = getObjectMaterialText(objectId);
                        if (objectText:find('(.+)\n{......}%$(.+)')) then
                            ...
                            result = {
                                ...
                                modelName = name or 'undefined',
                                fullStr = objectText .. ';' .. idPlateText
                            };
                        end
                    end
                end
