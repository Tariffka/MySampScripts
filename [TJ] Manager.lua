...
local managerNeedsUpdate = false
local managerCatalogItem = nil
...
function getLocalScriptVersion(filePath)
    ...
    -- Ищем паттерн script_version('X.X') или script_version("X.X")
    local version = content:match("script_version%(['\"](.-)['\"]%)")
    return version
end
        ...
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if file then
                ...
                if success then
                    ...
                    -- Проверяем обновление для самого менеджера
                    local myName = thisScript().filename
                    local myVersion = thisScript().version
        
                    for _, item in ipairs(scriptsCatalog) do
                        if item.filename == myName then
                            if item.version and item.version ~= myVersion then
                                managerNeedsUpdate = true
                                managerCatalogItem = item
                                chat("{FF9900}Script Manager: {FFFFFF}Доступна новая версия менеджера (" .. item.version .. ")! Откройте меню для обновления.")
                            else
...
local newFrame = imgui.OnFrame(
    ...
    function(player)
        ...
        if imgui.Begin("##jpt_main", renderWindow, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
            ...
            if managerNeedsUpdate and managerCatalogItem then
                imgui.Separator()
                if ColoredButton(fa("DOWNLOAD") .. " Обновить Script Manager", imgui.ImVec4(0.8, 0.2, 0.2, 1.0), imgui.ImVec2(-1, 30)) then
                    local myPath = getWorkingDirectory() .. "\\" .. thisScript().filename
                    local scriptNoCacheUrl = managerCatalogItem.url .. "?nocache=" .. os.time()
                    chat("{FF9900}Script Manager: {FFFFFF}Обновляюсь...")
                    downloadUrlToFile(scriptNoCacheUrl, myPath, function(id, status, p1, p2)
                        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                            chat("{FF9900}Script Manager: {FFFFFF}Успешно обновлен! Перезагружаю...")
                            thisScript():reload()
                        end
                    end)
                end
            ...
            for i, item in ipairs(scriptsCatalog) do
                ...
                if matchInstalled then
                    ...
                    local displayName = prefix .. item.name

                    -- Проверяем версию
                    local needsUpdate = false
                    if isInstalled and item.version then
                        local localVer = getLocalScriptVersion(filePath)
                        -- Если версии не совпадают (или локальная меньше), помечаем
                        if localVer and localVer ~= item.version then
                            displayName = displayName .. " (Нужно обновить)"
                            needsUpdate = true
                        end
                    end

                    imgui.PushStyleVarVec2(imgui.StyleVar.ItemSpacing, imgui.ImVec2(8, 8))
                    if imgui.Selectable(displayName, selectedScriptIdx == i) then
                        selectedScriptIdx = i
                    end
                    imgui.PopStyleVar()
                end
            ...
            if activeScript then
                ...
                if isInstalled then
                    local localVer = getLocalScriptVersion(filePath) or "Неизвестно"
                    local catVer = activeScript.version or "Не указана"
                    
                    imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), "Версия установленная: ")
                    imgui.SameLine()
                    
                    -- Если версии отличаются, подсвечиваем локальную красным
                    if catVer ~= "Не указана" and localVer ~= catVer then
                        imgui.TextColored(imgui.ImVec4(0.9, 0.2, 0.2, 1.0), localVer)
                        imgui.SameLine()
                        imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.2, 1.0), "(Доступна новая: " .. catVer .. ")")
                    else
                        imgui.Text(localVer)
                    end
                end
