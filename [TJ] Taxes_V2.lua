...
function main()
    ...
    math.randomseed(os.time()) -- Инициализация рандома
        ...
        if currentMinute == 0 then
            if not paydayTriggered then
                ...
                lua_thread.create(function()
                    ...
                    -- Задержка перед началом оплаты (от 15 до 50 секунд)
                    local randomDelay = math.random(15000, 50000)
                    wait(randomDelay) 
                    ...
                    if iniData.main.autoTaxes then
                        ...
                        -- Рандомная задержка между закрытием телефона и открытием фам меню (от 3 до 6 секунд)
...
if isPayingTaxes then return end
    ...
    lua_thread.create(function()
        ...
        -- Ждем 5 секунд. Если диалог не обработался и флаг всё еще true - сбрасываем.
        wait(5000)
...
function toggleFamHousePay()
    ...
    lua_thread.create(function()
        ...
        -- Защитный тайм-аут: ждем 5 секунд
        wait(5000)
        -- Если спустя 5 секунд флаг все еще true (диалог не обработался), сбрасываем его
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    ...
    if isPayingTaxes then
            ...
            if text:find(u8:decode("{ffff00}Оплата всех налогов{FFFFFF}")) then
                ...
                for line in text:gmatch("[^\r\n]+") do
                    if line:find(u8:decode("Оплата всех налогов")) then 
                        ...
                        -- УБРАЛИ ОТСЮДА СБРОС ФЛАГА, чтобы скрипт дождался следующего окна
                        return false
                    end
                    num = num + 1
                end
        ...
        -- Вот этот блок теперь успешно сработает, примет диалог и сам сбросит флаг
        elseif title == u8:decode("{BFBBBA}Оплата всех налогов") then
            sampSendDialogResponse(id, 1) -- Нажимаем "Оплатить/Да"
            isPayingTaxes = false -- Сбрасываем флаг только ПОСЛЕ подтверждения
    ...
    if isPayingFamHouse then
        ...
        -- Ищем, допуская разный регистр первой буквы
