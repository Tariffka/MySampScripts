...
imgui.OnFrame(
    ...
    function(cls)
        ...
        imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY), imgui.Cond.Always) -- Cond.Always гарантирует применение новых размеров
        ...
        if imgui.Begin('Auto AD | ' .. thisScript().version, _, imgui.WindowFlags.NoResize) then
            ...
            -- ЛЕВАЯ ПАНЕЛЬ (Конфиги): Заменено с -(resX / 3) на жесткую ширину 140
            imgui.BeginChild('::configChild', imgui.ImVec2(140, -65), true)
                for index, config in ipairs(settings.configs) do
                    if imgui.ButtonActivated(index == settings.main.activeConf, u8(config.name) .. "##" .. index, imgui.ImVec2(-1, 20)) then
            ...
            -- ЦЕНТРАЛЬНАЯ ПАНЕЛЬ (Выбор чата): Ширина 160
            ...
            -- ПРАВАЯ ПАНЕЛЬ (Настройки): Ширина 0 означает "занять всё оставшееся место", теперь она широкая
...
if imgui.Button(u8'Создать', imgui.ImVec2(0, -1)) then
