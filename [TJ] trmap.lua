...
function sampev.onServerMessage(_, text)
    if autoCopyEnabled and text:find(u8:decode('Территория найдена! После ее исчезновения она будет автоматически восстановлена')) then
        lua_thread.create(function()
            wait(200)
            sampAddChatMessage(sName .. u8:decode('Текст был обнаружен, производится автоматический /zonecopy'), -1)
            wait(400)
