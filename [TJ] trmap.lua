...
local sName = '{368b55}[Карта]{FFFFFF} – '
...
function main()
    ...
    wait(100)
    sampAddChatMessage(sName .. 'Используйте команды: /zonecopy, /zonepaste, /zonedel, /iskd, /autocopy', -1)
    ...
    sampRegisterChatCommand('autocopy', function()
        ...
        sampAddChatMessage(sName .. (autoCopyEnabled and 'Авто-копирование включено!' or 'Авто-копирование выключено!'), -1)
    end)
    ...
    sampRegisterChatCommand('iskd', function(arg)
        if kdcond == true then
            sampAddChatMessage('{FF0000}[Карта]{FFFFFF} – У тебя кулдаун. Ты не можешь использовать карту! Твой кулдаун кончится ' .. endkd, -1)
        elseif kdcond == false then
            sampAddChatMessage('{3cb043}[Карта]{FFFFFF} – Кулдауна нет. Можно использовать карту!', -1)
        end
    end)
    ...
    while true do
        ...
        if timenow == endkd then
            ...
            sampAddChatMessage('{3cb043}[Карта]{FFFFFF} – Кулдаун прошел! Карту можно использовать!', -1)
            kdcond = false
            endkd = 'Неизвестно.'
        end
    end
...
function sampev.onServerMessage(_, text)
    if autoCopyEnabled and text:find('Территория найдена! После ее исчезновения она будет автоматически восстановлена') then
        lua_thread.create(function()
            wait(200)
			sampAddChatMessage(sName .. 'Текст был обнаружен, производится автоматический /zonecopy', -1)
			wait(400)
